...
class VRMDriverHuawei(VRMDriver):
    ...
    def create_volume(self, volume):
        ...
        LOG.info(_("[CINDER-VRM] fc_vol_id [%s], name is %s ") % (fc_vol_id, name))
        ...
        try:
            self.volume_proxy.update_custom_properties(volume_urn=temp_str, external_uuid=volume['id'])
            # modify volume name
            self.volume_proxy.modify_volume(volume_id=fc_vol_id, name=name)
        except Exception as ex:
            LOG.error(
                _("[CINDER-VRM] modify volume information failed after create volume.volume id is [%s] ") % fc_vol_id)
            LOG.exception(ex)
        return model_update
    ...
    def delete_volume(self, volume):
        ...
        temp_str = vol_meta_dict.get('urn')
        LOG.info(_("[CINDER-VRM] urn is %s ,fc_volume_name is %s") %(temp_str, fc_volume_name))
        fc_vol_id = temp_str[temp_str.rfind(':') + 1:]
        try:
            self.volume_proxy.update_custom_properties(volume_urn=temp_str, external_uuid="")
            # modify volume name
            self.volume_proxy.modify_volume(volume_id=fc_vol_id, name=fc_volume_name)
        except Exception as ex:
            LOG.error(
                _("[CINDER-VRM] modify volume information failed after create volume.volume id is [%s] ") % fc_vol_id)
            LOG.exception(ex)
