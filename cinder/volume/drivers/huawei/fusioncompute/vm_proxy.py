...
class VmProxy(BaseProxy):
    ...
    def delete_vm(self, **kwargs):
        ...
        method = 'DELETE'
        path = self.site_uri + uri + '/' + kwargs.get('vm_id')
        if kwargs.get('isReserveDisks') is not None and kwargs.get('isReserveDisks') == 1:
    ...
    def check_template(self, **kwargs):
        ...
        for template in templates:
            if template_id is not None:
                ...
                id = urn[-10:]
                if id == template_id:
                    LOG.info(_("[VRM-CINDER] template exists [%s]"), template_id)
                    return id
    ...
    def create_volume_from_template(self, **kwargs):
        ...
        while True:
            ...
            LOG.info(_("[VRM-CINDER] template_vm status is %s"), template_vm.get('status'))
        ...
        try:
            ...
            if vm_id is None:
                ...
                if image_type == 'nfs':
                    ...
                    LOG.info(_("[VRM-CINDER] import_vm_from_nfs vm_urn is %s"), vm_urn)
                    vm_id = vm_urn[-10:]
                    ...
                    vm_urn = self.import_vm_from_uds(**kwargs)
                    kwargs.pop('linkClone')
                    LOG.info(_("[VRM-CINDER] import_vm_from_uds vm_urn is %s"), vm_urn)
                    vm_id = vm_urn[-10:]
                    kwargs['image_location'] = vm_id
                    ...
                    vm_urn = self.import_vm_from_glance(**kwargs)
                    kwargs.pop('linkClone')
                    LOG.info(_("[VRM-CINDER] import_vm_from_glance vm_urn is %s"), vm_urn)
                    vm_id = vm_urn[-10:]
                    kwargs['image_location'] = vm_id
    ...
    def export_volume_to_image(self, **kwargs):
        ...
        if vm_body is not None:
            ...
            LOG.info(_("[VRM-CINDER] volume_sequence_num %s"), kwargs['volume_sequence_num'])
