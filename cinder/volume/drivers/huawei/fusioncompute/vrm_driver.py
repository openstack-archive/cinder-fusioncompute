...
class VRMDriver(driver.VolumeDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if reserved_percentage is None:
            ...
            LOG.info(_("[VRM-CINDER] reserved_percentage [%s]"), reserved_percentage)
    ...
    def _get_host_datastore_vol(self):
        ...
        :return:
        '''
#        LOG.info(_("[VRM-CINDER] start get_host_datastore_vol()"))
        self.shared_hosts = []
        self.shared_datastores = []
        self.shared_volumes = []
...
#        LOG.info(_("[BRM-DRIVER] start _try_get_volume_stats() "))
        ...
        for pool in self.pool_list:
            ...
            if datastore.get('storageType') == 'advanceSan' and datastore.get('version') is not None:
...
#        LOG.info(_("[BRM-DRIVER] start get_volume_stats() "))
    ...
    def _choose_datastore(self, ds_meta):
        ...
        :return:
        '''
        #LOG.info(_("[BRM-DRIVER] start _choose_datastore() "))
        for datastore in self.SHARED_DATASTORES:
        ...
        if id:
            ...
            if type(volume_body) is not type(None):
                ...
                if len(replication_driver_data) > 0:
                    LOG.info(_("[BRM-DRIVER] replication_driver_data is %s"), sd_sn)
                    model_update['replication_driver_data'] = json.dumps(replication_driver_data)
                urn = volume_body['urn']
                uri = volume_body['uri']
                ...
                if storage_type is not None:
                    LOG.info(_("[BRM-DRIVER] the storage type of volume is %s"), storage_type)
    ...
    def _vrm_delete_volume(self, volume):
        ...
        if type(vol_metas) is not type(None):
            for meta in vol_metas:
                LOG.info(_("[BRM-DRIVER] volume_metadata is [%s:%s] "), meta.key, meta.value)
        ...
        if provider_location is None or provider_location == '':
            ...
            if vol_uri is not None:
                ...
                self._vrm_delete_volume_vm(volume_urn=fc_vol_urn, volume_uri=vol_uri)
            return
        fc_volume_urn, items = \
            self._vrm_unpack_provider_location(volume['provider_location'], 'urn')
        fc_vol_id = fc_volume_urn[fc_volume_urn.rfind(':') + 1:]
        self._check_replications(fc_vol_id)
        volume_uri, items = \
            self._vrm_unpack_provider_location(volume['provider_location'], 'uri')
        self._vrm_delete_volume_vm(volume_urn=fc_volume_urn, volume_uri=volume_uri)
    ...
    def _check_quick_start(self, volume_id):
        ...
        admin_metadata = self.db.volume_admin_metadata_get(admin_context, volume_id)
    ...
    def _check_replications(self, fc_vol_id):
        ...
        if fc_vol_id:
            try:
                LOG.info(_("[BRM-DRIVER] check replications : volume id in fc is %s"), fc_vol_id)
                body = self.volume_proxy.query_volume_replications(volume_id=fc_vol_id)
                LOG.info(_("[BRM-DRIVER] check replications : replications is %s"), body.get('replications'))
            except Exception as ex:
                LOG.error(_("[BRM-DRIVER] get volume replications expection"))
                LOG.exception(ex)
                return
            if body.get('replications'):
                LOG.error(_("[BRM-DRIVER] The volume %s to deleted has replications, "
        ...
        if volumes:
            for volume_ref in volumes:
                try:
                    ...
                    LOG.error(_("[BRM-DRIVER] delete_consistencygroup failed. "))
                    LOG.exception(ex)
    ...
    def create_volume(self, volume):
        ...
        if datastore:
            ...
            if volume_type_id:
                LOG.info(_("[BRM-DRIVER] query volume type id is %s"), volume_type_id)
                volume_type_extra_specs = self.db.volume_type_extra_specs_get(context, volume_type_id)
                LOG.info(_("[BRM-DRIVER] volume_type_extra_specs is %s"), str(volume_type_extra_specs))
                if volume_type_extra_specs and str(volume_type_extra_specs.get('hw:passthrough')).lower() == 'true':
                    args_dict['support_pvscsi'] = 1  # The volume created support direct io from vm to storage device.
            ...
            try:
                ...
                LOG.error(
                    _("[CINDER-VRM] get volume information failed after create volume.volume id is [%s] ") % fc_vol_id)
                LOG.exception(ex)
            return model_update
        else:
    ...
    def clone_image(self, context, volume, image_location, image_meta, image_service):
        ...
        if properties is not None:
            ...
            if quick_start is not None and str(quick_start).lower() == 'true':
                ...
                admin_context = cinder_context.get_admin_context()
                self.db.volume_admin_metadata_update(admin_context, volume.get('id'), {'quick_start': 'True'}, False)
                return None, False
        ...
        LOG.info(_('[BRM-DRIVER] image_location is %s') % args_dict['image_location'])
        ...
        args_dict['cluster_urn'] = self._get_cluster_by_dsurn(datastore.get('urn'))
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        if self._check_quick_start(volume.get('id')):
            ...
            if min_disk:
                ...
                msg = _('[BRM-DRIVER] image min_disk is none when create from quick start image')
                LOG.error(msg)
        ...
        if linked_clone:
            if not is_thin:
                ...
                raise exception.InvalidParameterValue(err=_('datastore does support linedclone'))
        args_dict['is_thin'] = is_thin

        args_dict['cluster_urn'] = self._get_cluster_by_dsurn(datastore.get('urn'))
        LOG.info(_("[BRM-DRIVER] self.cluster_urn [%s]") % args_dict['cluster_urn'])
                ...
                if prop:
                    ...
                    if os_type and os_version:
                        if os_type == 'Windows' and \
                                (os_version.startswith('Windows XP') or os_version.startswith('Windows Server 2003')):
                            msg = _(
                                "[BRM-DRIVER] volume  size must equal image min disk size while image min disk size is smaller than 4G and os is Windows XP or Windows Server 2003")
                            LOG.error(msg)
                            raise exception.ImageUnacceptable(image_id=image_id, reason=msg)
            ...
            if bucket_type is not None:
                if str(bucket_type) == 'wildcard':
                    ...
                    if project_tmp_id is None:
                        ...
                        raise exception.ParameterNotFound(param='project_id is none')
                    else:
                        uds_bucket_name += project_tmp_id

                LOG.info(_("uds_ip: %s, uds_port: %s, uds_bucket_name: %s"), FC_DRIVER_CONF.uds_ip,
                     FC_DRIVER_CONF.uds_port, uds_bucket_name)
            return '%s:%s:%s:%s' % (FC_DRIVER_CONF.uds_ip, FC_DRIVER_CONF.uds_port, uds_bucket_name, image_id)
        else:
            return None
    ...
    def copy_volume_to_image(self, context, volume, image_service, image_meta):
        ...
        image_service.update(context, image_id, image_property, purge_props=False)
        ...
        fc_vol_id = args_dict['volume_urn'][args_dict['volume_urn'].rfind(':') + 1:]
    ...
    def extend_volume(self,volume,new_size):
        ...
        if vol_uri is not None:
            ...
            if int(sizeGB) < int(new_size):
                self._check_datastore_capacity(vrm_volume, volume['size'], new_size)
    ...
    def _check_datastore_capacity(self, fc_volume, old_size, new_size):
        ...
        if datastore_urn:
            ...
            if free < extra_size:
                msg = _('The new size of the volume exceed the capacity of the datastore.')
                raise exception.ExtendVolumeError(message=msg)
...
snapshot db: CREATED_AT|UPDATED_AT|DELETED_AT| DELETED |ID |VOLUME_ID|USER_ID
  ...
  | DISPLAY_DESCRIPTION | PROVIDER_LOCATION | ENCRYPTION_KEY_ID | VOLUME_TYPE_ID | CGSNAPSHOT_ID
        ...
        LOG.info(('Creating a new backup for volume %s.') % vol_second_os['name'])
        kwargs = {}
        volume_file = {}
        ...
        if backup_list is not None:
            for back_tmp in backup_list:
                if back_tmp['status'] != "available" and back_tmp['status'] != "restoring":
                    continue
        ...
        if last_backup is None:
            ...
            if parent_snapshot_id_os is not None:
                ...
                if storage_type == 'advanceSan':
                    parent_snapshot_fc = self.volume_snapshot_proxy.query_volumesnapshot(uuid=parent_snapshot_id_fc)
                    if parent_snapshot_fc and parent_snapshot_fc.get('snapshotLunId') is not None:
                        volume_file['parent_snapshot_url'] = 'http://' + sd_sn \
                                                             + '/' + parent_snapshot_fc.get('snapshotLunId')
                else:
                    volume_file['parent_snapshot_url'] = 'http://' + ext_params_dic['dsMgntIp'] \
                                                         + '/' + ext_params_dic['dsResourceId'] \
                                                         + '/' + parent_snapshot_id_fc
            else:
                raise exception.InvalidVolumeMetadata(reason="snapshot_id is none")
        ...
        LOG.info(_("vol_source_fc linkCloneParent is %s") % vol_source_fc['linkCloneParent'])
        if vol_source_fc['linkCloneParent'] is not None:
            ...
            try:
                ...
                if storage_type == 'advanceSan':
                    volume_file['clone_volume_url'] = 'http://' + vol_linked_fc['storageSN'] \
                                                      + '/' + vol_linked_fc['lunId']
                else:
        ...
        LOG.info(_("[VRM-CINDER] volume_file [%s]"), volume_file)
        if storage_type == 'advanceSan':
            last_snapshot = self.volume_snapshot_proxy.query_volumesnapshot(uuid=last_snap_id_fc)
            if last_snapshot and last_snapshot.get('snapshotLunId') is not None:
                volume_file['snapshot_url'] = 'http://' + sd_sn \
                                              + '/' + last_snapshot.get('snapshotLunId')
        else:
    ...
    def restore_backup(self, context, backup, volume, backup_service):
        ...
        if backup['volume_id'] == volume['id']:  # restore backup to source volume
            ...
            last_snapshot_fc = self.volume_snapshot_proxy.query_volumesnapshot(uuid=last_snapshot_id_fc)
            if last_snapshot_fc:  # only the last backup has snapshot
                if storage_type == 'advanceSan':
                    volume_file['lastest_snapshot_url'] = 'http://' + vol_second_fc['storageSN'] + '/' + \
                                                          last_snapshot_fc['snapshotLunId']
                else:
                    volume_file['lastest_snapshot_url'] = 'http://' + ext_params_dic['dsMgntIp'] \
                                                      + '/' + ext_params_dic['dsResourceId'] \
                                                      + '/' + last_snapshot_id_fc
        ...
        if storage_type == 'advanceSan':
            ...
            if pvscsi_support is not None and pvscsi_support == 1:
                volume_file['storage_type'] = 5  # RDM: direct io volume
            volume_file['volume_url'] = 'http://' + vol_second_fc['storageSN'] + '/' + vol_second_fc['lunId']
        else:
            volume_file['storage_type'] = 0
        ...
        if vol_second_fc['linkCloneParent'] is not None:
            try:
                ...
                if vol_second_fc['storageType'] == 'advanceSan':
                    volume_file['clone_volume_url'] = 'http://' + vol_linked_fc['storageSN'] + vol_linked_fc['lunId']
                else:
                    linked_ext_params_dic = json.loads(vol_linked_fc.get('drExtParams'))
                    volume_file['clone_volume_url'] = 'http://' + linked_ext_params_dic['dsMgntIp'] \
                                                      + '/' + linked_ext_params_dic['dsResourceId'] \
    ...
    def delete_cgsnapshot(self, context, cgsnapshot):
        ...
        LOG.info(_('[BRM-DRIVER] start to delete cgsnapshot [%s]'), cgsnapshot_id)

        snapshots = self.db.snapshot_get_all_for_cgsnapshot(context, cgsnapshot_id)
        if snapshots and len(snapshots) > 0:
            for snapshot in snapshots:
                ...
                body = self.volume_snapshot_proxy.query_volumesnapshot(uuid=snapshot_uuid)
                if body is None:
                    LOG.warn(_('[BRM-DRIVER] snapshot [%s] is not exist while deleting cgsnapshot [%s]'), snapshot_uuid,
                             cgsnapshot_id)
                ...
                try:
                    ...
                    LOG.error(_('[BRM-DRIVER] delete snapshot [%s] error while deleting cgsnapshot [%s]'),
                              snapshot_uuid, cgsnapshot_id)
                    LOG.exception(ex)
    ...
    def create_cgsnapshot(self, context, cgsnapshot):
        ...
        if volumes is None or len(volumes) <= 0:
            ...
            for volume_ref in volumes:
                for snapshot in snapshots:
                    if snapshot['volume_id'] == volume_ref['id']:
                        ...
                        vol_urn = vol_meta_dict.get('urn')
                        if vol_urn is None:
                            LOG.warn(_("vol_urn  is null while creating cgsnapshot."))
                        snapshot_id = snapshot['id']
                        snapshot_uuid = str(snapshot_id).replace('-', '')
                        LOG.info(_("creating snapshot snapshot_uuid  is [%s], vol_urn is [%s]"), snapshot_uuid, vol_urn)
                        try:
                            body = self.volume_snapshot_proxy.create_volumesnapshot(snapshot_uuid=snapshot_uuid, vol_urn=vol_urn, enable_active=False)
                        except Exception as ex:
                            LOG.error(_("Trying to create snapshot failed, volume id is: %s"), snapshot['volume_id'])
                            LOG.exception(ex)
            ...
            try:
                self.volume_snapshot_proxy.active_snapshots(snapshot_uuids=snapshot_uuids)
            except Exception as ex:
                LOG.error(_("Trying to active snapshot failed, snapshot_uuids is: %s"), snapshot_uuids)
                LOG.exception(ex)
                for snapshot in snapshots:
                    snapshot['status'] = 'error'
                model_update['status'] = 'error'
    ...
    def _get_cluster_by_dsurn(self, ds_urn):
        ...
        for cluster in clusters:
            ...
            try:
                ...
                for datastore in datastores:
                    if datastore['urn'] == ds_urn:
                        hosts = self.cluster_proxy.list_hosts(clusterUrn=cluster_urn)
                        for host in hosts:
                            if host["status"] == "normal" and host["isMaintaining"] == False:
                                return cluster_urn
