...
#        LOG.info(_("[BRM-DRIVER] start get_volume_stats() "))
        ...
        if datastoreUrn:
            for datastore in self.SHARED_DATASTORES:
                if datastore['urn'] == datastoreUrn:
                    ...
                    LOG.info(_(
                        "[CINDER-BRM] datastore=%s found,but not satisfied "
                        "with [%s,%s,%d,%s]") % (
                                 datastoreUrn, storageType, str(isThin),
                                 quantityGB, hypervisorIp))
                    raise driver_exception.NotFound()
            raise driver_exception.NotFound()
        ds_hosts = None
        ds_urn = None
    ...
    def _vrm_unpack_provider_location(self, provider_location, key=None):
        ...
        kvalue = None
        kvs = {}
        if type(provider_location) is not type(None) and len(
                provider_location) > 0:
    ...
    def _check_replications(self, fc_vol_id):
        ...
        if fc_vol_id:
            try:
                LOG.info(_(
                    "[BRM-DRIVER] check replications : volume id in fc is %s"),
                         fc_vol_id)
    ...
    def create_cloned_volume(self, volume, src_volume):
        ...
        LOG.info(_("dest_volume_uri is %s, src_volume_uri is %s") % (
        dest_volume_uri, src_volume_uri))
        ...
        LOG.info(_("src_volume_id is [%s] dest vol id [%s]"), src_volume_id,
                 dest_volume_id)
    ...
    def clone_image(self, context, volume, image_location, image_meta,
        ...
        try:
            ...
            LOG.info(_(
                "[CINDER-BRM] host format exception, host is %s ") % volume.
                     get('host'))
            raise ex
    ...
    def copy_image_to_volume(self, context, volume, image_service, image_id):
        ...
        LOG.info(
            _("[BRM-DRIVER] %s image_type is %s") % (image_id, image_type))
                    ...
                    if os_type and os_version:
                        if os_type == 'Windows' and \
                                (os_version.startswith(
                                    ...
                                    'Windows Server 2003')):
                            msg = _(
                                "[BRM-DRIVER] volume  size must equal image min"
                                " disk size while image min disk size is "
    ...
    def _generate_image_location(self, image_id, context):
        ...
        if FC_DRIVER_CONF.export_image_type == 'nfs':
            ...
            if FC_DRIVER_CONF.uds_ip is not None and \
                            FC_DRIVER_CONF.uds_port is not None \
            ...
            if bucket_type is not None:
                ...
                LOG.info(_("uds_ip: %s, uds_port: %s, uds_bucket_name: %s"),
                         FC_DRIVER_CONF.uds_ip,
                     FC_DRIVER_CONF.uds_port, uds_bucket_name)
            return '%s:%s:%s:%s' % (
            FC_DRIVER_CONF.uds_ip, FC_DRIVER_CONF.uds_port, uds_bucket_name,
            image_id)
        else:
            return None
    ...
    def copy_volume_to_image(self, context, volume, image_service, image_meta):
        ...
        fc_vol_id = args_dict['volume_urn'][
                    args_dict['volume_urn'].rfind(':') + 1:]
...
snapshot db: CREATED_AT|UPDATED_AT|DELETED_AT| DELETED |ID |VOLUME_ID|USER_ID
  ...
  | DISPLAY_DESCRIPTION | PROVIDER_LOCATION | ENCRYPTION_KEY_ID | VOLUME_TYPE_ID | CGSNAPSHOT_ID
        ...
        if storage_type == 'advanceSan':
            ...
            volume_file['snapshot_url'] = 'http://' + ext_params_dic[
                'dsMgntIp'] \
                                          + '/' + ext_params_dic[
                                              'dsResourceId'] \
                                          + '/' + last_snap_id_fc
        ...
        if snapshots and len(snapshots) > 0:
            for snapshot in snapshots:
                ...
                try:
                    ...
                    LOG.error(_('[BRM-DRIVER] delete snapshot [%s] error while '
                                'deleting cgsnapshot [%s]'),
    ...
    def create_cgsnapshot(self, context, cgsnapshot):
        ...
        if volumes is None or len(volumes) <= 0:
                ...
                for snapshot in snapshots:
                    if snapshot['volume_id'] == volume_ref['id']:
                        ...
                        try:
                            ...
                            LOG.error(
                                _("Trying to create snapshot failed, volume id "
                                  "is: %s"), snapshot['volume_id'])
                            LOG.exception(ex)
            ...
            try:
                ...
                LOG.error(_(
                    "Trying to active snapshot failed, snapshot_uuids is: %s"),
                          snapshot_uuids)
                LOG.exception(ex)
                for snapshot in snapshots:
                    snapshot['status'] = 'error'
