...
#        LOG.info(_("[BRM-DRIVER] start get_volume_stats() "))
    ...
    def _vrm_delete_volume(self, volume):
        ...
        if vol_metas is not None:
            for meta in vol_metas:
                LOG.info(_("[BRM-DRIVER] volume_metadata is [%s:"
                           "%s] "),
                         meta.key, meta.value)
    ...
    def extend_volume(self, volume, new_size):
        ...
        if vol_metas is not None:
            for meta in vol_metas:
                msg = _("[BRM-DRIVER] volume_metadata is [%s:"
                        "%s] "), meta.key, meta.value
                LOG.info(msg)
    ...
    def delete_cgsnapshot(self, context, cgsnapshot):
        ...
        if snapshots and len(snapshots) > 0:
            for snapshot in snapshots:
                ...
                if body is None:
                    msg = _(
                        '[BRM-DRIVER] snapshot [%s] is not exist '
                        'while deleting cgsnapshot [%s]'),
                    snapshot_uuid,
                    cgsnapshot_id
                    LOG.warn(msg)
